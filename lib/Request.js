...
var Request = class Request {
    constructor(serverRequest) {
        ...
        this[urlParts] = (0, _url.parse)(serverRequest.url);
    ...
    *getFieldsAndFiles() {
        ...
        var originalRequest = this.getServerRequest();

        return new Promise(function (resolve, reject) {
            var form = _formidable2.default.IncomingForm();
            form.parse(originalRequest, function (error, parsedFields, parsedFiles) {
                if (error) {
                    reject(error);
                    return;
                }
